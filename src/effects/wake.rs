...
impl Effect for Wake {
    fn frames(&self, pixels: &[Rgb]) -> Box<dyn Iterator<Item = Vec<Rgb>> + Send + 'static> {
        ...
        let initial_color = self.start_color;
        let effect_end_brightness = self.end_brightness;
        ...
        Box::new(std::iter::from_fn(move || {
            ...
            let current_brightness = effect_end_brightness as f32 * progress;

            let target_color_at_full_brightness = initial_color.with_brightness(255);
            let interpolated_color =
                Rgb::lerp(&initial_color, target_color_at_full_brightness, progress);
